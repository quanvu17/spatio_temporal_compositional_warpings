...
# Load dataset 2
load("results/simulation_study_dataset2/dataset.rda")
...
if (i == 1){
  save(pred, file = "results/simulation_study_dataset2/statsep_predictions.rda")
}

## Stationary, Asymmetric model
d <- deepspat_nn_ST_GP(f = z ~ s1 + s2 + t - 1, data = df_train, g = ~ 1,
                       family = "exp_stat_asym",
                       layers_spat = layers_spat, layers_temp = layers_temp,
                       m = 50L,
                       order_id = order_id, nn_id = nn_id,
                       method = "REML", nsteps = 200L,
...
nn_id_pred <- FNN::get.knnx(data = locs_t, query = locs_new, k = 50)$nn.index
pred <- predict.deepspat_nn_ST_GP(d, newdata, nn_id_pred)
RMSPE_statasym <- RMSPE(df_test$y, pred$df_pred$pred_mean[1:nrow(df_test)])
CRPS_statasym <- CRPS(df_test$y,
     pred$df_pred$pred_mean[1:nrow(df_test)],
     pred$df_pred$pred_var[1:nrow(df_test)])
if (i == 1){
  save(pred, file = "results/simulation_study_dataset2/statasym_predictions.rda")
  
  # Save fitted warping parameters
  v_tf <- d$run(d$v_tf)
  save(v_tf, file = "results/simulation_study_dataset2/statasym_warpings.rda")
...
pred <- predict.deepspat_nn_ST_GP(d, newdata, nn_id_pred)
RMSPE_nonstatsep <- RMSPE(df_test$y, pred$df_pred$pred_mean[1:nrow(df_test)])
CRPS_nonstatsep <- CRPS(df_test$y,
     pred$df_pred$pred_mean[1:nrow(df_test)],
     pred$df_pred$pred_var[1:nrow(df_test)])
if (i == 1){
  save(pred, file = "results/simulation_study_dataset2/nonstatsep_predictions.rda")
}

## Nonstationary, Asymmetric model
d <- deepspat_nn_ST_GP(f = z ~ s1 + s2 + t - 1, data = df_train, g = ~ 1,
                       family = "exp_nonstat_asym",
                       layers_spat = layers_spat, layers_temp = layers_temp,
                       m = 50L,
                       order_id = order_id, nn_id = nn_id,
                       method = "REML", nsteps = 200L,
...
twarped_tf <- d$run(d$twarped_tf)
v_tf <- d$run(d$v_tf)
warping_param <- list(eta, eta_t, scalings, scalings_t, swarped_tf, twarped_tf, v_tf)
if (i == 1){
  save(warping_param, file = "results/simulation_study_dataset2/nonstatasym_warpings.rda")
}
...
nn_id_pred <- FNN::get.knnx(data = locs_warped, query = locs_new_warped, k = 50)$nn.index
pred <- predict.deepspat_nn_ST_GP(d, newdata, nn_id_pred)
RMSPE_nonstatasym <- RMSPE(df_test$y, pred$df_pred$pred_mean[1:nrow(df_test)])
CRPS_nonstatasym <- CRPS(df_test$y,
     pred$df_pred$pred_mean[1:nrow(df_test)],
     pred$df_pred$pred_var[1:nrow(df_test)])
if (i == 1){
  save(pred, file = "results/simulation_study_dataset2/nonstatasym_predictions.rda")
}

summary_results <- list(RMSPE_statsep, RMSPE_statasym, RMSPE_nonstatsep, RMSPE_nonstatasym,
                        CRPS_statsep, CRPS_statasym, CRPS_nonstatsep, CRPS_nonstatasym)
save(summary_results,
     file = paste0("results/simulation_study_dataset2/summary_", i, ".rda"))
